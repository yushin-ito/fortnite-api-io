...
class FortniteAPI extends BaseClient {
     ...
     * @param {string} [region=NAE]
     * @param {object} [options]
     * @param {string} [options.lang] en, ar, de, es, es-419, fr, it, ja, ko, pl, pt-BR, ru, tr
     * @returns {Promise<object>}
     */
    getTournaments(region = "NAE", options = {}) {
        const lang = options.lang || this.defaultLang;
        const uri = endpoints.getTournaments(region, lang);
        return this.request(uri);
    }
     ...
     * @returns {Promise<object>}
     */
    getTournamentSessionDetails(windowId) {
        const uri = endpoints.getTournamentSessionDetails(windowId);
        return this.request(uri);
    }
